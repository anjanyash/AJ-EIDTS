...
// ── Supabase ─────────────────────────────────────────────────────
const SUPA_URL = 'https://xxafdmlatewjjuzbzoru.supabase.co';
const SUPA_KEY = 'sb_publishable_4QRPNglmdyEFwThpnlJspg_1qnIIcYe';
...
async function sb(path, opts={}) {
  try {
    ...
    if(!res.ok){ const e=await res.text(); console.error('SB err',path,e); return null; }
    const t=await res.text(); return t?JSON.parse(t):[];
  } catch(e){ console.error('SB fetch fail',e); return null; }
}
...
const DB = {
  ...
  async register(email, password, name) {
    const existing = await sb('aj_users?email=eq.'+encodeURIComponent(email.toLowerCase())+'&select=id');
    if(existing && existing.length>0) return { error: 'Email already registered.' };
    const user = { id:'u_'+Date.now(), name, email:email.toLowerCase(), password, role:'client', joined:new Date().toISOString().slice(0,10), avatar:name.charAt(0).toUpperCase() };
    const r = await sb('aj_users', { method:'POST', body:JSON.stringify(user) });
    if(!r) return { error: 'Registration failed. Please try again.' };
    return { user };
  },
  async login(email, password) {
    // Ensure admin exists
    const admins = await sb('aj_users?email=eq.ajedits1455%40gmail.com&select=id');
    if(!admins || admins.length===0) {
      await sb('aj_users', { method:'POST', body:JSON.stringify({id:'admin_1',name:'AJ Admin',email:'ajedits1455@gmail.com',password:'admin123',role:'admin',joined:'2025-01-01',avatar:'A'}) });
    }
    const rows = await sb('aj_users?email=eq.'+encodeURIComponent(email.trim().toLowerCase())+'&select=*');
    if(!rows || rows.length===0) return { error: 'Invalid email or password.' };
    const user = rows[0];
    if(user.password !== password.trim()) return { error: 'Invalid email or password.' };
    return { user };
  },
  // ── Bookings ───────────────────────────────────────────────────
  async getBookings() {
    const r = await sb('aj_bookings?select=*&order=created_at.desc');
    return r||[];
  },
  async getUserBookings(uid) {
    const r = await sb('aj_bookings?user_id=eq.'+uid+'&select=*&order=created_at.desc');
    return r||[];
  },
  async saveBooking(b) {
    await sb('aj_bookings', { method:'POST', body:JSON.stringify(b) });
  },
  async updateBookingStatus(id, status) {
    await sb('aj_bookings?id=eq.'+id, { method:'PATCH', body:JSON.stringify({status}), prefer:'return=minimal' });
  },
  async deliverBooking(id, url, note) {
    await sb('aj_bookings?id=eq.'+id, { method:'PATCH', body:JSON.stringify({delivery_url:url, delivery_note:note, status:'Completed'}), prefer:'return=minimal' });
  },
  async rejectBooking(id, reason) {
    await sb('aj_bookings?id=eq.'+id, { method:'PATCH', body:JSON.stringify({status:'Rejected', reject_reason:reason||''}), prefer:'return=minimal' });
  },
  async cancelBooking(id) {
    await sb('aj_bookings?id=eq.'+id, { method:'PATCH', body:JSON.stringify({status:'Cancelled', cancelled_at:new Date().toISOString().slice(0,10)}), prefer:'return=minimal' });
  },
  // ── Payments ───────────────────────────────────────────────────
  async getPayments() {
    const r = await sb('aj_payments?select=*&order=created_at.desc');
    return r||[];
  },
  async addPayment(p) {
    await sb('aj_payments', { method:'POST', body:JSON.stringify(p) });
  },
  async updatePaymentStatus(bid, status) {
    await sb('aj_payments?booking_id=eq.'+bid, { method:'PATCH', body:JSON.stringify({status}), prefer:'return=minimal' });
  },
  // ── Samples ────────────────────────────────────────────────────
  async getSamples() {
    const r = await sb('aj_samples?select=*&order=added_on.desc');
    if(r && r.length>0) return r;
    // Seed defaults if empty
    const defaults=[{id:'s1',title:'Wedding Highlight',type:'Wedding',thumb:'🎬',added_on:'2025-01-10'},{id:'s2',title:'Birthday Reel',type:'Event',thumb:'🎉',added_on:'2025-02-14'},{id:'s3',title:'Brand Promo',type:'Promo',thumb:'📽️',added_on:'2025-03-01'},{id:'s4',title:'Instagram Pack',type:'Reel',thumb:'📱',added_on:'2025-04-05'}];
    await sb('aj_samples', { method:'POST', body:JSON.stringify(defaults), prefer:'return=minimal' });
    return defaults;
  },
  async addSample(s) {
    await sb('aj_samples', { method:'POST', body:JSON.stringify(s) });
  },
  async removeSample(id) {
    await sb('aj_samples?id=eq.'+id, { method:'DELETE', prefer:'return=minimal' });
  },
  // ── Users/Profiles ─────────────────────────────────────────────
  async getProfiles() {
    const r = await sb('aj_users?role=eq.client&select=*');
    return r||[];
  },
  // ── Reviews ────────────────────────────────────────────────────
  async getReviews() {
    const r = await sb('aj_reviews?select=*&order=date.desc');
    return r||[];
  },
  async addReview(rev) {
    await sb('aj_reviews', { method:'POST', body:JSON.stringify(rev) });
  },
  // ── Notifications ──────────────────────────────────────────────
  async getNotifications() {
    const r = await sb('aj_notifications?select=*&order=date.desc');
    return r||[];
  },
  async addNotification(n) {
    await sb('aj_notifications', { method:'POST', body:JSON.stringify(n) });
  },
  async clearNotifications() {
    await sb('aj_notifications', { method:'DELETE', prefer:'return=minimal' });
  },
  // ── Settings ───────────────────────────────────────────────────
  async changePassword(email, oldPass, newPass) {
    const rows = await sb('aj_users?email=eq.'+encodeURIComponent(email)+'&select=*');
    if(!rows||rows.length===0||rows[0].password!==oldPass) return {error:'Current password is incorrect.'};
    await sb('aj_users?email=eq.'+encodeURIComponent(email), { method:'PATCH', body:JSON.stringify({password:newPass}), prefer:'return=minimal' });
    return {success:true};
  },
};
...
const G = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  @keyframes borderGlow{0%,100%{box-shadow:0 0 8px rgba(201,168,76,0.3)}50%{box-shadow:0 0 20px rgba(201,168,76,0.7)}}
  @keyframes glowPulse{0%,100%{text-shadow:0 0 30px rgba(201,168,76,0.3)}50%{text-shadow:0 0 60px rgba(201,168,76,0.6)}}
...
function Particles() {
  ...
  return (
    <div style={{position:'absolute',inset:0,overflow:'hidden',pointerEvents:'none'}}>
      {pts.map((p,i)=>(
        <div key={i} style={{position:'absolute',left:p.left+'%',top:p.top+'%',width:p.size,height:p.size,borderRadius:'50%',background:'var(--gold)',opacity:0.3,animation:'pulse '+p.dur+'s ease-in-out infinite',animationDelay:p.delay+'s',opacity:0.25}}/>
      ))}
...
function IntroScreen({onComplete}){
  ...
  return(
    <div style={{position:'fixed',inset:0,zIndex:9999,background:'radial-gradient(ellipse at 30% 70%,#050d20 0%,#02040a 70%)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',opacity:ph===3?0:1,transition:ph===3?'opacity 1.2s ease':'none'}}>
      <Particles/>
      <div style={{position:'absolute',top:0,left:0,right:0,height:'1px',background:'linear-gradient(90deg,transparent,var(--gold),transparent)',animation:'scanline 3s linear infinite',opacity:0.6}}/>
      {[320,480,640].map((sz,i)=><div key={i} style={{position:'absolute',width:sz,height:sz,border:'1px solid rgba(201,168,76,0.'+(8-i*2)+')',borderRadius:'50%',animation:'spin '+(15+i*10)+'s linear infinite'+(i%2?' reverse':''),opacity:0.15}}/>)}
      <div style={{textAlign:'center',position:'relative',zIndex:1}}>
        <div style={{opacity:ph>=1?1:0,transform:ph>=1?'scale(1)':'scale(0.5)',transition:'all 0.8s cubic-bezier(0.34,1.56,0.64,1)',marginBottom:32}}>
          <div style={{width:88,height:88,border:'1px solid rgba(201,168,76,0.4)',margin:'0 auto',display:'flex',alignItems:'center',justifyContent:'center',animation:ph>=1?'borderGlow 2s infinite':'none'}}>
            <span className="fd" style={{fontSize:30,fontWeight:700,color:'var(--gold)',letterSpacing:'0.1em'}}>AJ</span>
          </div>
        </div>
        <p style={{fontFamily:'Montserrat',fontSize:'10px',fontWeight:500,letterSpacing:'0.5em',color:'rgba(245,240,232,0.4)',marginBottom:20,opacity:ph>=1?1:0,transition:'opacity 1s ease 0.3s'}}>WELCOME TO</p>
        <h1 className="fd" style={{fontSize:'clamp(48px,12vw,110px)',fontWeight:700,letterSpacing:'0.3em',color:'var(--white)',animation:'glowPulse 2s ease-in-out infinite',lineHeight:1,opacity:ph>=1?1:0,transition:'opacity 0.8s ease 0.5s'}}>AJ EDITS</h1>
        <div style={{marginTop:24,opacity:ph>=2?1:0,transform:ph>=2?'translateY(0)':'translateY(20px)',transition:'all 0.8s ease'}}>
          <p className="fs" style={{fontSize:'clamp(14px,2.5vw,20px)',fontStyle:'italic',color:'var(--gold)',fontStyle:'italic'}}>Turning Moments Into Cinematic Masterpieces</p>
        </div>
        <div style={{marginTop:48,width:'200px',height:'1px',background:'rgba(255,255,255,0.1)',margin:'48px auto 0',overflow:'hidden'}}>
...
function Nav({setPage,page,user,setUser,newBookingsCount}) {
  ...
  return (
    <>
      <nav style={{position:'fixed',top:0,left:0,right:0,zIndex:1000,padding:'16px clamp(20px,5vw,60px)',display:'flex',justifyContent:'space-between',alignItems:'center',background:sc||menuOpen?'rgba(2,4,10,0.95)':'transparent',backdropFilter:'blur(24px)',borderBottom:(sc||menuOpen)?'1px solid var(--border)':'none',transition:'all 0.4s'}}>
        <button onClick={()=>go('home')} style={{background:'none',border:'none',cursor:'pointer',display:'flex',alignItems:'center',gap:10}}>
          <div style={{width:32,height:32,display:'flex',alignItems:'center',justifyContent:'center'}}>
            <span className="fd" style={{fontSize:12,fontWeight:700,color:'#fff',letterSpacing:'0.05em'}}>AJ</span>
          </div>
          <span className="fd" style={{fontSize:16,fontWeight:700,color:'var(--white)',letterSpacing:'0.15em'}}>EDITS</span>
        </button>
        {/* Desktop nav */}
...
function Hero({setPage}) {
  ...
  return (
    <section id="home" style={{minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'120px clamp(20px,8vw,120px) 80px',position:'relative',overflow:'hidden',background:'radial-gradient(ellipse at 50% 60%,rgba(201,168,76,0.04),transparent 70%)'}}>
      <Particles/>
      {/* Decorative lines */}
      <div style={{position:'absolute',left:0,top:'20%',width:'30%',height:1,background:'linear-gradient(90deg,transparent,rgba(201,168,76,0.15))',pointerEvents:'none'}}/>
      <div style={{position:'absolute',right:0,bottom:'30%',width:'25%',height:1,background:'linear-gradient(270deg,transparent,rgba(201,168,76,0.1))',pointerEvents:'none'}}/>
      <div style={{textAlign:'center',position:'relative',zIndex:1,maxWidth:900}}>
        ...
        <h1 className="fd" style={{fontSize:'clamp(40px,10vw,92px)',fontWeight:700,letterSpacing:'0.04em',lineHeight:1.0,marginBottom:8,
          background:'linear-gradient(135deg,var(--white) 0%,var(--gold) 40%,var(--white) 60%,var(--gold2) 100%)',backgroundSize:'200% auto',
          WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',animation:'shimmer 8s linear infinite',
          opacity:vis?1:0,transform:vis?'none':'translateY(40px)',transition:'opacity 0.8s ease 0.2s, transform 0.8s ease 0.2s'}}>
          TURNING MOMENTS
        </h1>
        <h1 className="fd" style={{fontSize:'clamp(40px,10vw,92px)',fontWeight:700,letterSpacing:'0.04em',lineHeight:1.1,marginBottom:36,
          color:'var(--gold)',
          WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',animation:'shimmer 8s linear infinite reverse',
          opacity:vis?1:0,transform:vis?'none':'translateY(40px)',transition:'opacity 0.8s ease 0.3s, transform 0.8s ease 0.3s'}}>
          INTO MASTERPIECES
        </h1>
        <p className="fs" style={{fontSize:'clamp(15px,2vw,20px)',fontStyle:'italic',color:'var(--dim)',maxWidth:520,margin:'0 auto 52px',lineHeight:1.8,opacity:vis?1:0,transform:vis?'none':'translateY(20px)',transition:'all 0.8s ease 0.4s'}}>
          Premium cinematic editing for weddings, reels, promos and more
        </p>
        ...
        <div style={{marginTop:72,display:'flex',gap:0,justifyContent:'center',flexWrap:'wrap',opacity:vis?1:0,transition:'opacity 0.8s ease 0.8s'}}>
          {[['500+','Projects'],['100%','Satisfaction'],['5★','Rating']].map(([v,l],i)=>(
            <div key={l} style={{textAlign:'center',padding:'0 40px',borderRight:i<2?'1px solid var(--border)':'none'}}>
              <div className="fd" style={{fontSize:'clamp(22px,4vw,36px)',color:'var(--gold)',fontWeight:700,letterSpacing:'0.05em'}}>{v}</div>
              <div style={{fontSize:10,letterSpacing:'0.3em',color:'var(--dim)',marginTop:6,fontFamily:'Rajdhani',fontWeight:600}}>{l.toUpperCase()}</div>
            </div>
          ))}
        </div>
      </div>
...
function Services() {
  return (
    <section id="services" style={{padding:'clamp(80px,10vw,120px) clamp(20px,5vw,80px)',background:'linear-gradient(to bottom,transparent,rgba(10,31,69,0.3) 50%,transparent)'}}>
      <div style={{textAlign:'center',marginBottom:64}}>
        <span className="section-label" style={{justifyContent:'center'}}>What We Offer</span>
        <h2 className="section-title" style={{marginTop:16,}}>OUR SERVICES</h2>
      </div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:2}}>
        {SERVICES.map((s,i)=>(
          <div key={i} style={{padding:'40px 36px',display:'flex',alignItems:'flex-start',gap:24,background:i%2===0?'rgba(8,13,26,0.9)':'rgba(10,16,32,0.9)',border:'1px solid var(--border)',transition:'all 0.3s',cursor:'default',position:'relative',overflow:'hidden'}}
            onMouseEnter={e=>{e.currentTarget.style.borderColor='var(--border2)';e.currentTarget.style.background='rgba(10,31,69,0.5)';}}
            onMouseLeave={e=>{e.currentTarget.style.borderColor='var(--border)';e.currentTarget.style.background=i%2===0?'rgba(8,13,26,0.9)':'rgba(10,16,32,0.9)';}}>
            <div style={{display:'none'}}/>
            <div style={{fontSize:40,flexShrink:0,animation:'float 4s ease-in-out infinite',animationDelay:i*0.3+'s',lineHeight:1,}}>{s.icon}</div>
            <div>
              <h3 className="fd" style={{fontSize:16,fontWeight:700,letterSpacing:'0.1em',marginBottom:10,color:'var(--white)'}}>{s.title.toUpperCase()}</h3>
              <p style={{fontSize:13,color:'var(--dim)',lineHeight:1.8,fontWeight:300}}>{s.desc}</p>
            </div>
          </div>
        ))}
      </div>
...
function ClientDashboard({user,setPage}) {
  ...
  const book=async()=>{
    ...
    if(!form.desc.trim()) errs.desc='Please describe your project so we know what you need.';
    if(!form.driveLink.trim()) errs.driveLink='Please share your Google Drive link with raw footage.';
    if(!form.phone.trim()) errs.phone='Please enter your phone/WhatsApp number so we can reach you.';
    if(Object.keys(errs).length>0){
      setFieldErrors(errs);
      setMsg('⚠️ Please fill in all required fields before submitting.');
      return;
    }
    setFieldErrors({});
    setSaving(true);
  ...
  const [cancelConfirm,setCancelConfirm]=useState(null); // booking id awaiting confirm
  const cancelBooking=async(id)=>{
    ...
    await DB.addNotification({
      ...
      message:`${user.name} cancelled their "${b?.service}" booking (ID: #${id.slice(-8)})`,
      booking_id:id,
    ...
    setBookings(p=>p.map(bk=>bk.id===id?{...bk,status:'Cancelled',cancelled_at:new Date().toISOString().slice(0,10)}:bk));
  ...
  const statusC=s=>({Completed:'var(--green)','Payment Received':'var(--green)','In Progress':'#a0a0ff','Pending Payment':'var(--yellow)',Rejected:'#ff6b6b',Cancelled:'rgba(255,255,255,0.3)'}[s]||'var(--dim)');
  const canCancel=s=>['Pending Payment','Work Pending'].includes(s);
            ...
            <div style={{display:'flex',flexDirection:'column',gap:16}}>
              {bookings.map((b,i)=>(
                <div key={b.id} className="card" style={{padding:'28px 32px'}}>
                  ...
                  {b.status==='Cancelled'&&(
                    <div style={{marginBottom:14,padding:'14px 18px',background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.1)'}}>
                      <div style={{fontSize:10,letterSpacing:'0.2em',color:'rgba(255,255,255,0.4)',marginBottom:4}}>🚫 BOOKING CANCELLED</div>
                      <div style={{fontSize:12,color:'var(--dim)'}}>Cancelled on {b.cancelled_at||b.created_at}. Contact us to rebook.</div>
                    </div>
                  )}
                  {/* Cancel confirm dialog */}
                  {cancelConfirm===b.id&&(
                    <div style={{marginBottom:14,padding:'18px 20px',background:'rgba(255,107,107,0.08)',border:'1px solid rgba(255,107,107,0.35)',borderRadius:2}}>
                      <div style={{fontSize:11,fontWeight:600,letterSpacing:'0.15em',color:'#ff6b6b',marginBottom:8}}>⚠ CANCEL THIS BOOKING?</div>
                      <div style={{fontSize:12,color:'var(--dim)',marginBottom:16,lineHeight:1.6}}>Are you sure you want to cancel <span style={{color:'var(--white)'}}>{b.service}</span>? This cannot be undone.</div>
                      <div style={{display:'flex',gap:10}}>
                        <button onClick={()=>cancelBooking(b.id)} style={{padding:'10px 24px',background:'#ff6b6b',color:'#fff',border:'none',fontFamily:'Syncopate',fontSize:9,fontWeight:700,letterSpacing:'0.2em',cursor:'pointer'}}>YES, CANCEL</button>
                        <button onClick={()=>setCancelConfirm(null)} style={{padding:'10px 24px',background:'transparent',color:'var(--dim)',border:'1px solid var(--border)',fontFamily:'Syncopate',fontSize:9,letterSpacing:'0.2em',cursor:'pointer'}}>KEEP IT</button>
                      </div>
                    </div>
                  )}
                  <div style={{borderTop:'1px solid var(--border)',paddingTop:16,display:'flex',gap:8,alignItems:'flex-end',flexWrap:'wrap'}}>
                    ...
                    <div style={{marginLeft:'auto'}}>
                      {canCancel(b.status)&&cancelConfirm!==b.id&&(
                        <button onClick={()=>setCancelConfirm(b.id)} style={{padding:'8px 20px',background:'transparent',color:'rgba(255,107,107,0.7)',border:'1px solid rgba(255,107,107,0.3)',fontFamily:'Montserrat',fontSize:10,letterSpacing:'0.15em',cursor:'pointer',transition:'all 0.3s'}}
                          onMouseEnter={e=>{e.target.style.background='rgba(255,107,107,0.1)';e.target.style.color='#ff6b6b';}}
                          onMouseLeave={e=>{e.target.style.background='transparent';e.target.style.color='rgba(255,107,107,0.7)';}}>
                          CANCEL BOOKING
                        </button>
                      )}
                    </div>
      ...
      {!loading&&tab==='new booking'&&!payStep&&(
        <div style={{maxWidth:560}}>
          <div className="card" style={{padding:40}}>
            ...
            {msg&&<div style={{padding:'12px 16px',background:msg.includes('saved')?'rgba(0,200,100,0.1)':'rgba(255,100,100,0.1)',border:'1px solid '+(msg.includes('saved')?'rgba(0,200,100,0.3)':'rgba(255,100,100,0.3)'),color:msg.includes('saved')?'var(--green)':'#ff6b6b',fontSize:12,marginBottom:20,lineHeight:1.6}}>{msg}</div>}

            <label style={{fontSize:10,letterSpacing:'0.2em',color:fieldErrors.service?'#ff6b6b':'var(--dim)',display:'block',marginBottom:8}}>SERVICE <span style={{color:'#ff6b6b'}}>*</span></label>
            <select className="input-field" value={form.service} onChange={e=>{setForm(f=>({...f,service:e.target.value}));setFieldErrors(fe=>({...fe,service:''}));}} style={{marginBottom:4,background:'rgba(255,255,255,0.03)',borderColor:fieldErrors.service?'rgba(255,107,107,0.6)':''}}>
            ...
            {fieldErrors.service&&<div style={{fontSize:11,color:'#ff6b6b',marginBottom:12,display:'flex',alignItems:'center',gap:6}}><span>⚠</span>{fieldErrors.service}</div>}
            {!fieldErrors.service&&<div style={{marginBottom:16}}/>}

            <label style={{fontSize:10,letterSpacing:'0.2em',color:fieldErrors.desc?'#ff6b6b':'var(--dim)',display:'block',marginBottom:8}}>DESCRIPTION <span style={{color:'#ff6b6b'}}>*</span></label>
            <textarea className="input-field" rows={3} placeholder="Describe your project — what style, mood, length, any references..." value={form.desc} onChange={e=>{setForm(f=>({...f,desc:e.target.value}));setFieldErrors(fe=>({...fe,desc:''}));}} style={{marginBottom:4,resize:'vertical',borderColor:fieldErrors.desc?'rgba(255,107,107,0.6)':''}}/>
            {fieldErrors.desc&&<div style={{fontSize:11,color:'#ff6b6b',marginBottom:12,display:'flex',alignItems:'center',gap:6}}><span>⚠</span>{fieldErrors.desc}</div>}
            {!fieldErrors.desc&&<div style={{marginBottom:16}}/>}

            <label style={{fontSize:10,letterSpacing:'0.2em',color:fieldErrors.driveLink?'#ff6b6b':'var(--dim)',display:'block',marginBottom:8}}>GOOGLE DRIVE LINK <span style={{color:'#ff6b6b'}}>*</span> <span style={{color:'rgba(201,168,76,0.4)',fontWeight:300}}>(raw footage)</span></label>
            <input className="input-field" type="url" placeholder="https://drive.google.com/..." value={form.driveLink} onChange={e=>{setForm(f=>({...f,driveLink:e.target.value}));setFieldErrors(fe=>({...fe,driveLink:''}));}} style={{marginBottom:4,borderColor:fieldErrors.driveLink?'rgba(255,107,107,0.6)':''}}/>
            {fieldErrors.driveLink&&<div style={{fontSize:11,color:'#ff6b6b',marginBottom:4,display:'flex',alignItems:'center',gap:6}}><span>⚠</span>{fieldErrors.driveLink}</div>}
            <p style={{fontSize:10,color:'rgba(245,240,232,0.25)',marginBottom:16,lineHeight:1.6}}>Make sure link is set to <span style={{color:'var(--gold)'}}>Anyone with link can view</span></p>

            <div style={{background:'rgba(201,168,76,0.03)',border:'1px solid '+(fieldErrors.phone?'rgba(255,107,107,0.4)':'rgba(201,168,76,0.12)'),padding:'20px 20px 4px',marginBottom:16}}>
              <div style={{fontSize:10,letterSpacing:'0.3em',color:'var(--gold)',marginBottom:16}}>CONTACT DETAILS</div>
              <label style={{fontSize:10,letterSpacing:'0.2em',color:fieldErrors.phone?'#ff6b6b':'var(--dim)',display:'block',marginBottom:8}}>PHONE / WHATSAPP <span style={{color:'#ff6b6b'}}>*</span></label>
              <input className="input-field" type="tel" placeholder="e.g. 9876543210" value={form.phone} onChange={e=>{setForm(f=>({...f,phone:e.target.value}));setFieldErrors(fe=>({...fe,phone:''}));}} style={{marginBottom:4,borderColor:fieldErrors.phone?'rgba(255,107,107,0.6)':''}}/>
              {fieldErrors.phone&&<div style={{fontSize:11,color:'#ff6b6b',marginBottom:12,display:'flex',alignItems:'center',gap:6}}><span>⚠</span>{fieldErrors.phone}</div>}
              {!fieldErrors.phone&&<div style={{marginBottom:16}}/>}
              <label style={{fontSize:10,letterSpacing:'0.2em',color:'var(--dim)',display:'block',marginBottom:8}}>ADDITIONAL NOTE <span style={{color:'rgba(201,168,76,0.4)',fontWeight:300}}>(optional)</span></label>
            ...
            <div style={{fontSize:10,color:'rgba(245,240,232,0.3)',marginBottom:12,letterSpacing:'0.1em'}}>Fields marked <span style={{color:'#ff6b6b'}}>*</span> are required</div>
...
function AdminDashboard({user}) {
  ...
  const load=useCallback(async()=>{
    setLoading(true);
    const [b,u,s,p,notifs]=await Promise.all([DB.getBookings(),DB.getProfiles(),DB.getSamples(),DB.getPayments(),DB.getNotifications()]);
    setBookings(b||[]);setProfiles(u||[]);setSamples(s||[]);setPayments(p||[]);setNotifications(notifs||[]);
    const seen=parseInt(localStorage.getItem('admin_seen_bookings')||'0')||0;
    setSeenCount(seen);
    setLoading(false);
  },[]);
  useEffect(()=>{load();},[]);
  // Poll every 30s for new cancellations
  useEffect(()=>{
  ...
  useEffect(()=>{
    if(tab==='bookings'&&bookings.length>0){
      localStorage.setItem('admin_seen_bookings',bookings.length);
      setSeenCount(bookings.length);
    }
  },[tab,bookings.length]);
  const newCount=Math.max(0,bookings.length-seenCount);
  const cancelNotifs=notifications.filter(n=>n.type==='cancellation'&&!n.read);
  const totalCancelNotifs=notifications.filter(n=>n.type==='cancellation').length;
  const markNotifsRead=async()=>{
    const all=await DB.getNotifications();
    for(const n of all){ if(!n.read) await sb('aj_notifications?id=eq.'+n.id,{method:'PATCH',body:JSON.stringify({read:true}),prefer:'return=minimal'}); }
    const updated=all.map(n=>({...n,read:true}));
    setNotifications(updated);
  };
  const totalRev=payments.filter(p=>p.status==='Paid').reduce((a,b)=>a+b.amount,0);
  const sc=t=>({background:tab===t?'rgba(201,168,76,0.15)':'transparent',border:'1px solid '+(tab===t?'rgba(201,168,76,0.4)':'var(--border)'),color:tab===t?'var(--gold)':'var(--dim)'});
  const sColor=s=>({'Pending Payment':'var(--yellow)','Payment Received':'var(--green)','In Progress':'#a0a0ff','Completed':'var(--green)','Paid':'var(--green)','Pending':'var(--yellow)','Work Pending':'#7090ff','Payment Under Review':'var(--yellow)','Rejected':'#ff6b6b','Cancelled':'rgba(255,255,255,0.35)'}[s]||'var(--dim)');
  const invoice=b=>{
    ...
    <div style={{minHeight:'100vh',padding:'clamp(100px,12vw,120px) clamp(20px,5vw,60px) 60px'}}>
        ...
        <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
          {['overview','bookings','payments','samples','users','settings'].map(t=>(
            <button key={t} onClick={()=>{setTab(t);setMsg('');}} style={{fontSize:9,letterSpacing:'0.2em',fontFamily:'Montserrat',fontWeight:500,textTransform:'uppercase',padding:'10px 18px',cursor:'pointer',transition:'all 0.3s',position:'relative',...sc(t)}}>
              {t.toUpperCase()}
              {t==='bookings'&&(newCount>0||cancelNotifs.length>0)&&(
                <span style={{position:'absolute',top:-6,right:-6,background:'#ff6b6b',color:'#fff',fontSize:8,fontWeight:700,borderRadius:'50%',width:16,height:16,display:'flex',alignItems:'center',justifyContent:'center',animation:'bounceIn 0.4s ease'}}>{newCount+cancelNotifs.length}</span>
              )}
              {t==='overview'&&cancelNotifs.length>0&&(
                <span style={{position:'absolute',top:-6,right:-6,background:'#ff6b6b',color:'#fff',fontSize:8,fontWeight:700,borderRadius:'50%',width:16,height:16,display:'flex',alignItems:'center',justifyContent:'center',animation:'bounceIn 0.4s ease'}}>{cancelNotifs.length}</span>
              )}
            </button>
          ))}
      ...
      {!loading&&tab==='overview'&&(
        <div>
          ...
          {cancelNotifs.length>0&&(
            <div style={{marginBottom:24,padding:'20px 24px',background:'rgba(255,107,107,0.06)',border:'1px solid rgba(255,107,107,0.3)',borderLeft:'4px solid #ff6b6b'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
                <div style={{display:'flex',alignItems:'center',gap:10}}>
                  <span style={{fontSize:18}}>🚫</span>
                  <div className="fd" style={{fontSize:11,letterSpacing:'0.2em',color:'#ff6b6b'}}>CANCELLATION ALERTS — {cancelNotifs.length} NEW</div>
                </div>
                <button onClick={async()=>{
                  ...
                  setNotifications(updated);
                  load();
                }} style={{background:'transparent',border:'1px solid rgba(255,107,107,0.4)',color:'rgba(255,107,107,0.8)',fontFamily:'Montserrat',fontSize:9,letterSpacing:'0.15em',padding:'6px 14px',cursor:'pointer'}}>MARK ALL READ</button>
              </div>
              <div style={{display:'flex',flexDirection:'column',gap:10}}>
                {cancelNotifs.map((n,i)=>(
                  <div key={i} style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:8,padding:'12px 16px',background:'rgba(255,107,107,0.04)',border:'1px solid rgba(255,107,107,0.15)'}}>
                    <div>
                      <div style={{fontSize:12,color:'var(--white)',marginBottom:3}}>{n.message}</div>
                      <div style={{fontSize:10,color:'rgba(245,240,232,0.35)'}}>{n.date} · Rs.{n.price}</div>
                    </div>
                    <span style={{fontSize:9,letterSpacing:'0.2em',padding:'4px 10px',background:'rgba(255,107,107,0.15)',color:'#ff6b6b',border:'1px solid rgba(255,107,107,0.3)',flexShrink:0}}>CANCELLED</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:16,marginBottom:28}}>
            {[{l:'Total Bookings',v:bookings.length,i:'📋',c:'var(--gold)'},{l:'Revenue',v:'Rs.'+totalRev.toLocaleString(),i:'💰',c:'var(--green)'},{l:'Pending Payments',v:payments.filter(p=>p.status==='Pending').length,i:'⏳',c:'var(--yellow)'},{l:'Clients',v:profiles.length,i:'👥',c:'var(--gold)'},{l:'Cancelled',v:bookings.filter(b=>b.status==='Cancelled').length,i:'🚫',c:'#ff6b6b'},{l:'Paid',v:payments.filter(p=>p.status==='Paid').length,i:'✅',c:'var(--green)'}].map((s,i)=>(
      ...
      {!loading&&tab==='bookings'&&(
        <div style={{overflowX:'auto'}}>
          {cancelNotifs.length>0&&(
            <div style={{marginBottom:20,padding:'14px 20px',background:'rgba(255,107,107,0.07)',border:'1px solid rgba(255,107,107,0.3)',borderLeft:'4px solid #ff6b6b',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:12}}>
              <div style={{display:'flex',alignItems:'center',gap:10}}>
                <span style={{fontSize:20}}>🚫</span>
                <div>
                  <div style={{fontSize:11,letterSpacing:'0.2em',color:'#ff6b6b',fontWeight:600}}>{cancelNotifs.length} NEW CANCELLATION{cancelNotifs.length>1?'S':''}</div>
                  <div style={{fontSize:11,color:'var(--dim)',marginTop:2}}>{cancelNotifs.map(n=>n.client_name).join(', ')} cancelled their booking{cancelNotifs.length>1?'s':''}</div>
                </div>
              </div>
              <button onClick={async()=>{
                const all=await DB.getNotifications();
                ...
                const updated=all.map(n=>({...n,read:true}));
                setNotifications(updated);
              }} style={{background:'transparent',border:'1px solid rgba(255,107,107,0.4)',color:'rgba(255,107,107,0.8)',fontFamily:'Montserrat',fontSize:9,letterSpacing:'0.15em',padding:'7px 16px',cursor:'pointer',flexShrink:0}}>MARK READ</button>
            </div>
          )}
          <p style={{color:'var(--dim)',fontSize:12,marginBottom:20}}>{bookings.length} total bookings · {bookings.filter(b=>b.status==='Cancelled').length} cancelled</p>
          {bookings.length===0?<div style={{textAlign:'center',padding:60,color:'var(--dim)'}}>No bookings yet.</div>:(
            <div style={{display:'flex',flexDirection:'column',gap:16}}>
              {bookings.map((b,i)=>(
                <div key={i} className="card" style={{padding:'24px 28px',opacity:b.status==='Cancelled'?0.7:1,borderColor:b.status==='Cancelled'?'rgba(255,107,107,0.25)':''}}>
                  {b.status==='Cancelled'&&(
                    <div style={{marginBottom:14,padding:'10px 16px',background:'rgba(255,107,107,0.08)',border:'1px solid rgba(255,107,107,0.25)',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:8}}>
                      <div style={{display:'flex',alignItems:'center',gap:8}}>
                        <span>🚫</span>
                        <div>
                          <div style={{fontSize:10,letterSpacing:'0.2em',color:'#ff6b6b',fontWeight:600}}>BOOKING CANCELLED BY CLIENT</div>
                          <div style={{fontSize:11,color:'var(--dim)',marginTop:2}}>Cancelled on {b.cancelled_at||b.created_at} · No action needed</div>
                        </div>
                      </div>
                      <span style={{fontSize:9,letterSpacing:'0.15em',padding:'4px 12px',background:'rgba(255,107,107,0.15)',color:'#ff6b6b',border:'1px solid rgba(255,107,107,0.3)',flexShrink:0}}>CANCELLED</span>
                    </div>
                  )}
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',flexWrap:'wrap',gap:12,marginBottom:16}}>
                    <div>
